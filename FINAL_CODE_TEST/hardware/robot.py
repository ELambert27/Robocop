    ...
    def go(self):
        ...
        while flag:
            ...
            if self.orientation == 0:
                orientation_print = '0-north'
            elif self.orientation == 1:
                orientation_print = '1-east'
            elif self.orientation == 2:
                orientation_print = '2-south'
            elif self.orientation == 3:
                orientation_print = '3-west'
            print('current orientation:', orientation_print)
            ...
            if self.map.nodes[self.position].is_visited != 2:
                ...
                if len(self.location_stack) == 0:
                    ...
                    if len(self.map.nodes[self.position].available_direction) != 0:
                        self.turn_to_direction(self.map.nodes[self.position].available_direction[0]) 
                        self.map.nodes[self.position].available_direction.remove(self.map.nodes[self.position].available_direction[0])
                    else:
                        self.map.nodes[self.position].is_visited = 2
                    """
            print('---------------- loop done ----------------\n')
